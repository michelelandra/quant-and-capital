...
type Position = {
  ...
  price: number;      // prezzo di carico
  note?: string;
  date: string;       // yyyy-mm-dd
};
type HistoryPoint = { date: string; port: number; sp: number };
...
export default function PortfolioPage() {
  ...
  /* % performance vs inizio (10 k) */
  const portPct = ((equity / INITIAL_CASH) - 1) * 100;

  /* calcola % S&P 500 rispetto al primo SPY salvato ------------------ */
  const baseSpy = typeof window !== 'undefined'
    ? Number(localStorage.getItem(SPY_BASE_KEY) || 0)
    : 0;
  useEffect(() => {
  ...
  /* --------- salva nello storico giornaliero ----------------------- */
  useEffect(() => {
    if (!spyPrice) return; // aspetta prezzo SPY valido
    if (typeof window === 'undefined') return;

    const saved: HistoryPoint[] =
  ...
  /* --------- carica equityHistory per grafico ---------------------- */
  const equityHistory: HistoryPoint[] = useMemo(() => {
    if (typeof window === 'undefined') return [];
    return JSON.parse(localStorage.getItem(HISTORY_KEY) || '[]');
  }, [portPct, spPct]); // ricomputa quando cambiano
  ...
  const handleAdd = (e: FormEvent) => {
    ...
    if (qty > 0 && cost > cash) { alert('Not enough cash'); return; }

    setCash(c => c + (qty < 0 ? cost : -cost));
    setHistory(h => [
      ...h,
      { id: uuidv4(), ticker:ticker.toUpperCase(), qty, price, note, date: today },
    ]);
    setTicker(''); setQty(0); setPrice(0); setNote('');
  };

  const resetDay = () => {
    const keep = history.filter(p => p.date !== today);
    const refund = history.filter(p=>p.date===today)
      .reduce((s,p)=>s+Math.abs(p.qty)*p.price*(p.qty>0?1:-1),0);
    setHistory(keep);
    setCash(c=>c+refund);
  };
  const resetAll = () => {
    setHistory([]); setCash(INITIAL_CASH);
  ...
  return (
    <main className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <table className="w-full text-sm border-collapse">
        <thead className="border-b">
          <tr className="text-left">
            <th>Ticker</th><th>Qty</th><th>Avg.</th><th>Current</th><th>P/L</th><th>Note</th>
          </tr>
        </thead>
        <tbody>
          {rows.map(r=>(
      ...
      <ResponsiveContainer width="100%" height={250}>
        <PieChart>
          <Pie data={rows.map(r=>({ticker:r.ticker,value:Math.abs(r.qty)*r.current}))}
               dataKey="value" nameKey="ticker" isAnimationActive={false}
