...
async function getReactionState(postId: string, visitorId?: string | null) {
  const { data, error } = await supabaseService
    .from("analyses_reactions")
  ...
  if (visitorId) {
    const mine = reactions.find((r) => r.visitor_id === visitorId);
    myReaction = mine?.reaction ?? 0;
  }
...
export async function GET(req: Request) {
  try {
    ...
    const state = await getReactionState(postId, visitorId);
